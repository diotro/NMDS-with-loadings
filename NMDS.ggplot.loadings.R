...
#Ok that looks way better.  Now let's try it in ggplot
...
#Here I am using diet, but you could also use any other metadata correlated to your samples
Treatment<-CHEM$Treatment
#Here we will try to use the PC1 scale of chem.variables.PC1 to scale the size of the points by.
p<-ggplot(ord.scrs)+geom_point(mapping = aes(x = NMDS1, y = NMDS2, colour=Treatment, shape=Treatment, size=chem.variables.PC1))
p
